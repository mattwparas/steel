...
impl Analysis {
    ...
    pub fn run_with_scope(&mut self, exprs: &[ExprKind], clear_scope: bool) {
        ...
        // log::info!("Analysis Count: {}", expr_count);
...
impl ScopeInfo {
    ...
    pub fn new_top_level(id: SyntaxObjectId) -> Self {
        Self {
            id,
            captured: false,
            usage_count: 0,
    ...
    pub fn new_local(id: SyntaxObjectId, offset: usize, depth: u16) -> Self {
        Self {
            id,
            captured: false,
            usage_count: 0,
...
fn define_var(
    ...
    let id = define.name.atom_syntax_object().unwrap();
    scope.define(
        *id.ty.identifier().unwrap(),
        ScopeInfo::new_top_level(id.syntax_object_id),
    );
}
...
impl<'a> AnalysisPass<'a> {
    fn get_captured_vars(
        ...
        // log::info!("{}", self.info.scope.iter_without_bottom().count());
        // log::info!("{}", self.info.scope.iter_top().count());

        // TODO: Build an index against captured variables, so that
        // we don't have to iterate over everything in order to find
    ...
    fn visit_func_args(&mut self, lambda_function: &LambdaFunction, depth: usize) {
        ...
        for (index, arg) in lambda_function.args.iter().enumerate() {
            ...
            if heap_alloc {
                ...
                self.info.scope.define(
                    *name,
                    ScopeInfo::new_local(id, index, self.info.scope.depth() as _),
                );
...
impl<'a> VisitorMutUnitRef<'a> for AnalysisPass<'a> {
    ...
    fn visit_let(&mut self, l: &'a crate::parser::ast::Let) {
        ...
        for (index, arg) in l.local_bindings().enumerate() {
            ...
            if heap_alloc {
                ...
                self.info.scope.define(
                    *name,
                    ScopeInfo::new_local(id, stack_offset, self.info.scope.depth() as _),
                );
    ...
    fn visit_atom(&mut self, a: &'a crate::parser::ast::Atom) {
        ...
        if let Some(ident) = name {
            ...
            let current_depth = self.info.scope.depth();
            let root_info = self.info.scope.get_mut(ident);
            ...
            if let Some(depth) = root_info.as_ref().and_then(|x| x.depth) {
                if depth == 1 {
                    ...
                    return;
                } else if depth as usize == current_depth {
                    ...
                    let mut_ref = root_info.unwrap();

                    // log::info!("moved up Key at top, depth: {:?}", mut_ref.depth);
            ...
            //     log::info!("Key at top, depth: {:?}", mut_ref.depth);
            //     // Not sure if this is going to be a problem...
            //     // Because if its captured at this stage, I think we want it to be marked as captured
            //     // TODO:
            //     // mut_ref.captured = false;
            ...
            // patch those vars in
            if let Some(is_captured) = root_info {
...
impl<'a> LiftClosuresToGlobalScope<'a> {
    ...
    fn perform_closure_lifting(&mut self, ol: &mut Let) {
        ...
        if let ExprKind::Let(l) = &mut ol.body_expr {
            // Collect each of the bindings that might be potentially
            // lifted and captured, before we figure out what the things
                        ...
                        if !function_info.captured_vars().is_empty() && info.usage_count == 1 {
                            ...
                            if let ExprKind::Begin(b) = &mut l.body_expr {
                                let mut found_escape_checker: Option<
                                    CheckIdentifierOnlyOccursInUnboxCallPosition,
                                > = None;
                                    ...
                                    if let ExprKind::List(l) = expr {
                                        if l.first_ident().copied() == Some(*SETBOX) {
                                            if l.third_ident() == Some(function_name) {
                                                ...
                                                for (ident, _) in ol.bindings.iter() {
                ...
                if let ExprKind::LambdaFunction(r) = expression {
                    ...
                    if !function_info.captured_vars().is_empty() && info.usage_count == 1 {
                        ...
                        if let ExprKind::Begin(b) = &mut l.body_expr {
                                ...
                                if let ExprKind::List(l) = expr {
                                    if l.first_ident().copied() == Some(*SETBOX) {
                                        if l.third_ident() == Some(function_name) {
                                            ...
                                            // Check that the original func name appeared in the outer lets
                                            for (outer_index, (ident, _)) in
                                                ol.bindings.iter().enumerate()
                                            {
                                                let name = ident.atom_identifier().unwrap();
                                                ...
                                                if original_func_name == *name {
                                                    ...
                                                    found_escape_checker = Some(escape_checker);
                            ...
                            // If we've made it to this point, then we're good to go ahead and try:
                            for (index, expr) in b.exprs.iter_mut().enumerate() {
                                if let ExprKind::List(l) = expr {
                                    if l.first_ident().copied() == Some(*SETBOX) {
                                        if l.third_ident() == Some(function_name) {
                            ...
                            if let Some(index_to_remove) = index_to_remove {
                                ...
                                replacers.push((replacer, callsite_modifier));
                            }
                        }
                    }
                }
            ...
            if changed {
                while let Some((outer, inner)) = outer_inner_indexes.pop() {
                    ol.bindings.remove(outer);
...
impl<'a> VisitorMutUnitRef<'a> for CheckIdentifierOnlyOccursInUnboxCallPosition {
...
impl<'a> VisitorMutRefUnit for LiftClosuresToGlobalScope<'a> {
    fn visit_let(&mut self, ol: &mut Let) {
        ...
        */
        ...
        //
        // (let ((...)) (let ((...)) exprs ...))
        self.perform_closure_lifting(ol);
                ...
                if let ExprKind::LambdaFunction(l) = lambda {
                    if let Some(info) = self.analysis.get_function_info(l) {
                        ...
                        if info.captured_vars.is_empty() {
                            ...
                            let mut dummy = ExprKind::List(List::new(ThinVec::new()));
...
fn box_argument(ident: ExprKind) -> ExprKind {
    ExprKind::List(List::new(thin_vec![ExprKind::atom("#%box"), ident]))
}

fn unbox_argument(ident: ExprKind) -> ExprKind {
    ExprKind::List(List::new(thin_vec![ExprKind::atom("#%unbox"), ident]))
}

fn setbox_argument(ident: ExprKind, expr: ExprKind) -> ExprKind {
    ExprKind::List(List::new(thin_vec![
        ExprKind::atom("#%set-box!"),
        ident,
        expr
    ]))
}
...
impl<'a> VisitorMutRefUnit for ReplaceSetOperationsWithBoxes<'a> {
    fn visit(&mut self, expr: &mut ExprKind) {
        match expr {
            ...
            ExprKind::Set(s) => {
                ...
                let mut set_expr = ExprKind::List(List::empty());
                core::mem::swap(&mut s.expr, &mut set_expr);

                let mut dummy_ident = ExprKind::List(List::empty());
    ...
    fn visit_lambda_function(&mut self, lambda_function: &mut LambdaFunction) {
        ...
        if let Some(function_info) = function_info {
            let mut mutable_variables = ThinVec::new();
            ...
            if !mutable_variables.is_empty() {
                let mut body = ExprKind::List(List::empty());
                ...
                let mut new_expr = ThinVec::with_capacity(mutable_variables.len() + 1);
...
struct CollectReferences {
    idents: rustc_hash::FxHashSet<InternedString>,
}
...
impl<'a> SemanticAnalysis<'a> {
    ...
    pub(crate) fn remove_unused_globals_with_prefix(
        ...
        let mut collected = CollectReferences {
            idents: rustc_hash::FxHashSet::default(),
        };
    ...
    pub fn replace_no_argument_lets_with_body(&mut self) -> &mut Self {
        ...
        let func = |_: &Analysis, anon: &mut ExprKind| {
            if let ExprKind::Let(l) = anon {
                if l.bindings.is_empty() {
                    let mut dummy = ExprKind::List(List::empty());
