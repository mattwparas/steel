...
use crate::gc::Gc;
// Bring in alloc/core types regardless of std availability
use alloc::vec::Vec;
...
fn immutable_vector_rest(vector: &mut SteelVal) -> Result<SteelVal> {
    match vector {
        SteelVal::VectorV(SteelVector(v)) => match Gc::get_mut(v) {
            Some(v) => {
                v.pop_front();
                Ok(mem::replace(vector, SteelVal::Void))
            }
...
fn make_immutable_vector(mut rest: RestArgsIter<'_, &SteelVal>) -> Result<SteelVal> {
    ...
    let vector: Vector<_> = iter::repeat(())
        .map(|_| fill.clone())
        .take(len as usize)
        .collect();
    ...
    match vector {
        SteelVal::VectorV(SteelVector(v)) => match Gc::get_mut(v) {
            Some(v) => {
                ...
                match back {
                    Some(back) => {
                        let vector = mem::replace(vector, SteelVal::Void);
    ...
    fn make_vector_impl(ctx: &mut VmCore, args: &[SteelVal]) -> Result<SteelVal> {
        match &args {
            &[SteelVal::IntV(i)] if *i >= 0 => {
                Ok(ctx.make_mutable_vector_iter(iter::repeat(SteelVal::IntV(0)).take(*i as usize)))
            }
            &[SteelVal::IntV(i), initial_value] if *i >= 0 => {
                ...
                Ok(ctx.make_mutable_vector_iter(
                    iter::repeat(initial_value.clone()).take(*i as usize),
                ))
            }
            _ => {
