...
(define (#%reader-macro-map env char)
  (~> *map* (hash-get env) (hash-get char)))

(define (#%add-to-reader-map env char quoted-expr)
  (displayln ">> inside the kernel <<")
  (define env-map (or (hash-try-get *map* env) (hash)))
  (define once-added (hash-insert env-map char (eval quoted-expr)))

  (displayln "Evaluated function:")
  (displayln once-added)

  (set! *map* (hash-insert *map* env once-added))

  (displayln "Finished updating the reader macro map"))
