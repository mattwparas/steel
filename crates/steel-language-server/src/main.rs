...
struct Backend {
    client: Client,
    ast_map: DashMap<String, Vec<ExprKind>>,
    document_map: DashMap<String, Rope>,
    // TODO: This needs to hold macros to help with resolving definitions
    _macro_map: DashMap<String, HashMap<InternedString, SteelMacro>>,

    ignore_set: Arc<DashSet<InternedString>>,
    globals_set: Arc<DashSet<InternedString>>,

    defined_globals: DashSet<String>,
}
    ...
    async fn goto_definition(
        ...
        let definition = async {
            ...
            if location != uri {
                ...
                // Read the file in
                // let expression = tokio::fs::read_to_string(&location.to_file_path().unwrap())
                //     .await
                //     .unwrap();
...
impl Backend {
    async fn on_change(&self, params: TextDocumentItem) {
        ...
        let diagnostics = {
            ...
            // let mut base = self.prelude.clone();
...
fn make_error(mut diagnostic: Diagnostic) -> Diagnostic {
    diagnostic.severity = Some(DiagnosticSeverity::ERROR);
    diagnostic
}

struct ExternalModuleResolver {
