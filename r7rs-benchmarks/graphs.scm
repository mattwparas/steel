...
(define fold
  (lambda (lst folder state)
    ...
    ;;            folder)
    (do ((lst lst (cdr lst)) (state state (folder (car lst) state))) ((null? lst) state))))
...
(define vector-fold
  (lambda (vec folder state)
    ...
    (let ([len (vector-length vec)])
      (do ((i 0 (+ i 1)) (state state (folder (vector-ref vec i) state))) ((= i len) state)))))
...
(define gnatural-fold
  (lambda (limit folder state)
    ...
    ;;        (assert (procedure? folder)
    ;;            folder)
    (do ((i 0 (+ i 1)) (state state (folder i state))) ((= i limit) state))))
...
(define there-exists?
  (lambda (lst ok?)
    ...
    ;;            ok?)
    (let _-*- ([lst lst])
      (and (not (null? lst)) (or (ok? (car lst)) (_-*- (cdr lst)))))))
          ...
          (let _-**- ([in universe]
            ...
            (let* ([first (car in)]
                   ...
                   [accross (if (null? rest)
                                accross
                                (lambda (new-t-state) (_-**- rest (cons first out) new-t-state)))])
              (b-folder first
                        b-state
                        t-state
      ...
      (let _-*- ([y 0])
        (if (= x y)
            ...
            (let ([x->y? (vector-ref from-x y)]
              ...
              (cond
                [(eq? x->y? (vector-ref from-perm-x perm-y))
                 (let ([y->x? (vector-ref (vector-ref graph y) x)])
                   (cond
                     [(eq? y->x? (vector-ref (vector-ref graph perm-y) perm-x)) (_-*- (+ y 1))]
                     [y->x? 'less]
                     [else 'more]))]
                [x->y? 'less]
                [else 'more])))))))
    ...
    (let* ([root (- size 1)]
           ...
           [non-root-minimal?
            (let ([cont (lambda (perm state accross)
                          ...
                          ;;                                        state)
                          (accross #t))])
              (lambda (size) (minimal-folder size edge? cont #t)))]
           [root-minimal? (let ([cont (lambda (perm state accross)
                                        ...
                                        (case (cmp-next-vertex edge? perm root root)
                                          ...
                                          ;;                                            (assert #f)
                                          [else (error #f "???")]))])
                            (lambda () (minimal-folder root edge? cont #t)))])
      (let _-*- ([vertex 0]
                 [state state])
        (cond
          [(not (non-root-minimal? vertex)) state]
          [(= vertex root)
           ;;                        (assert
           ;;                            (begin
             ...
             (let _-*- ([v 0]
               ...
               (cond
                 ...
                 [(and (natural-for-all? root
                                         (lambda (v)
                                           (there-exists? efrr (lambda (r) (vector-ref r v)))))
                       (root-minimal?))
             ...
             (let _-**- ([sv 0]
               ...
               (if (= sv vertex)
                   (begin
                     ...
                     (_-*- (+ vertex 1) state))
                   (let* (;; no sv->vertex, no vertex->sv
                          [state (_-**- (+ sv 1) outs state)]
                                 ...
                                 (let* (;; sv->vertex, no vertex->sv
                                        ...
                                        [state (if (= outs max-out)
                                                   ...
                                                   (begin
                                                     ...
                                                     (let (;; sv->vertex, vertex->sv
                                                           [state (_-**- (+ sv 1) (+ outs 1) state)])
                                                       (vector-set! edges
                                                                    vertex
