...
(define (internal-node-name node)
  (vector-ref node 0))
(define (internal-node-green-edges node)
  (vector-ref node 1))
(define (internal-node-red-edges node)
  (vector-ref node 2))
(define (internal-node-blue-edges node)
  (vector-ref node 3))
(define (set-internal-node-name! node name)
  (vector-set! node 0 name))
(define (set-internal-node-green-edges! node edges)
  (vector-set! node 1 edges))
(define (set-internal-node-red-edges! node edges)
  (vector-set! node 2 edges))
(define (set-internal-node-blue-edges! node edges)
...
(define (make-edge-setter mutator!)
  (lambda (node value)
    (cond
      [(any-node? node) (error #f "Can't set edges from the ANY node")]
      [(none-node? node) 'OK]
...
(define (blue-edge-operation edge)
  (vector-ref edge 0))
(define (blue-edge-arg-node edge)
  (vector-ref edge 1))
(define (blue-edge-res-node edge)
  (vector-ref edge 2))
(define (set-blue-edge-operation! edge value)
  (vector-set! edge 0 value))
(define (set-blue-edge-arg-node! edge value)
  (vector-set! edge 1 value))
(define (set-blue-edge-res-node! edge value)
  (vector-set! edge 2 value))
...
(define (internal-graph-nodes graph)
  (vector-ref graph 0))
(define (internal-graph-already-met graph)
  (vector-ref graph 1))
(define (internal-graph-already-joined graph)
  (vector-ref graph 2))
(define (set-internal-graph-nodes! graph nodes)
  (vector-set! graph 0 nodes))
...
(define (canonicalize-graph graph classes)
  (define (fix node)
    (define (fix-set object selector mutator)
      (mutator object
               (map (lambda (node) (find-canonical-representative node classes)) (selector object))))
  ...
  (define (fix-table table)
    ...
    (define (filter-and-fix predicate-fn update-fn list)
      (let loop ([list list])
        (cond
          [(null? list) '()]
          [(predicate-fn (car list)) (cons (update-fn (car list)) (loop (cdr list)))]
          [else (loop (cdr list))])))
    (define (fix-line line)
      (filter-and-fix (lambda (entry) (canonical? (car entry)))
                      (lambda (entry)
    ...
    (if (null? table)
        ...
        (cons (car table)
              (filter-and-fix (lambda (entry) (canonical? (car entry)))
                              ...
                              (cdr table)))))
  (make-internal-graph (map (lambda (class) (fix (car class))) classes)
...
(define (none-node? node)
  (eq? node none-node))

(define any-node (make-node 'any '()))
(define (any-node? node)
  (eq? node any-node))
       ...
       (let loop ([blues (blue-edges t2)])
         (if (null? blues)
             ...
             (let* ([current-edge (car blues)]
               ...
               (and (has-op? phi t1)
                    (does-conform (res (sig phi t1)) (res (sig phi t2)))
                    (does-conform (arg (sig phi t2)) (arg (sig phi t1)))
                    (loop (cdr blues))))))]))
    ...
    (if (null? nodes)
        ...
        (let ([this-node (car nodes)])
          (define (add-node classes)
            (cond
              [(null? classes) (list (list this-node))]
              [(equivalent? this-node (caar classes))
               (cons (cons this-node (car classes)) (cdr classes))]
              [else (cons (car classes) (add-node (cdr classes)))]))
...
(define (find-canonical-representative element classification)
  (let loop ([classes classification])
    (cond
      [(null? classes) (error #f "Can't classify" element)]
...
(define (xreduce graph)
  (let ([classes (classify (graph-nodes graph))]) (canonicalize-graph graph classes)))
...
(define (meet graph node1 node2)
  (cond
    ...
    [else
     (let ([result
            (make-node
             (string-append "(" (internal-node-name node1) " ^ " (internal-node-name node2) ")"))])
       (add-graph-nodes! graph result)
       (insert! (already-met graph) node1 node2 result)
       (set-blue-edges!
        result
        (map (lambda (op) (blue-edge-operate join meet graph op (sig op node1) (sig op node2)))
...
(define (join graph node1 node2)
  (cond
    ...
    [else
     (let ([result
            (make-node
             (string-append "(" (internal-node-name node1) " v " (internal-node-name node2) ")"))])
       (add-graph-nodes! graph result)
       (insert! (already-joined graph) node1 node2 result)
       (set-blue-edges!
        result
        (map (lambda (op) (blue-edge-operate meet join graph op (sig op node1) (sig op node2)))
...
(define (test a0 b0 c0 d0)
  (setup a0 b0 c0 d0)
  (map internal-node-name (graph-nodes (make-lattice (make-graph a b c d any-node none-node) #f))))
