...
(define alist
  (quote ((x f (plus (plus a b) (plus c (zero)))) (y f (times (times a b) (plus c d)))
...
(let ()
    ...
    (add-lemma-lst
     (quote
      ((equal (compile form) (reverse (codegen (optimize form) (nil))))
       ...
       (equal (power-eval (big-plus (power-rep i base) (power-rep j base) (zero) base) base)
              (plus i j))
  ...
  (define (add-lemma term)
    (cond
      [(and (pair? term) (eq? (car term) (quote equal)) (pair? (cadr term)))
       (put (car (cadr term))
            (quote lemmas)
            (cons (translate-term term) (get (car (cadr term)) (quote lemmas))))]
  ...
  (define (translate-term term)
    (cond
      [(not (pair? term)) term]
      [else (cons (symbol->symbol-record (car term)) (translate-args (cdr term)))]))

  (define (translate-args lst)
    (cond
      [(null? lst) '()]
      [else (cons (translate-term (car lst)) (translate-args (cdr lst)))]))
  ...
  (define (untranslate-term term)
    (cond
      [(not (pair? term)) term]
      [else (cons (get-name (car term)) (map untranslate-term (cdr term)))]))
  ...
  (define (tautologyp x true-lst false-lst)
    (cond
      ...
      [(eq? (car x) if-constructor)
       (cond
         [(truep (cadr x) true-lst) (tautologyp (caddr x) true-lst false-lst)]
         [(falsep (cadr x) false-lst) (tautologyp (cadddr x) true-lst false-lst)]
         [else
  ...
  (define (scons x y original)
    (if (and (eq? x (car original)) (eq? y (cdr original))) original (cons x y)))
  ...
  (define (rewrite term)
    ...
    (cond
      [(not (pair? term)) term]
      [else
       (rewrite-with-lemmas (scons (car term) (rewrite-args (cdr term)) term)
                            (get-lemmas (car term)))]))

  (define (rewrite-args lst)
    (cond
      [(null? lst) '()]
      [else (scons (rewrite (car lst)) (rewrite-args (cdr lst)) lst)]))
  ...
  (define (one-way-unify1-lst lst1 lst2)
    (cond
      [(null? lst1) (null? lst2)]
      [(null? lst2) #f]
      [(one-way-unify1 (car lst1) (car lst2)) (one-way-unify1-lst (cdr lst1) (cdr lst2))]
      [else #f]))
  ...
  (define (trans-of-implies n)
    (translate-term (list (quote implies) (trans-of-implies1 n) (list (quote implies) 0 n))))

  (define (trans-of-implies1 n)
    (cond
      [(equal? n 1) (list (quote implies) 0 1)]
      [else (list (quote and) (list (quote implies) (- n 1) n) (trans-of-implies1 (- n 1)))]))
  ...
  (define (term-args-equal? lst1 lst2)
    (cond
      [(null? lst1) (null? lst2)]
      [(null? lst2) #f]
      [(term-equal? (car lst1) (car lst2)) (term-args-equal? (cdr lst1) (cdr lst2))]
      [else #f]))
...
(with-input-from-file (bench-input "sboyer") run-benchmark)
