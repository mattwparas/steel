              ...
              (if (pair? rules)
                  (let ([rule (car rules)])
                    (let loop ([l rule]
                               [nts nts])
                      (if (pair? l)
                          (let ([nt (car l)]) (loop (cdr l) (add-nt nt nts)))
  ...
  (let* ([nts (non-terminals grammar)] ;; id map = list of non-terms
    ...
    (define (setup-tables grammar nts starters enders predictors steps names)
      ...
      (define (add-conf conf nt nts class)
        (let ([i (ind nt nts)]) (vector-set! class i (cons conf (vector-ref class i)))))
                        ...
                        (let* ([ender (car l1)]
                          ...
                          (if ender-set
                              (let loop2 ([k (conf-set-head ender-set)]
                                          [l2 l2])
                                (if (>= k 0)
                                    (if (and (>= k i) (conf-set-member? (vector-ref states k) prev i))
                                        (let ([prev-trees (deriv-trees prev
                                                                       i
                                                                       k
                                                                       enders
                                                                       steps
                                              ...
                                              [ender-trees (deriv-trees ender
                                                                        ...
                                                                        states
                                                                        nb-nts)])
                                          (let loop3 ([l3 ender-trees]
                                                      [l2 l2])
                                            (if (pair? l3)
                                                (let ([ender-tree (list (car l3))])
                                                  (let loop4 ([l4 prev-trees]
                                                              [l2 l2])
                                                    (if (pair? l4)
                                                        (loop4 (cdr l4)
                                                               (cons (append (car l4) ender-tree) l2))
                                                        (loop3 (cdr l3) l2))))
                    ...
                    (if (pair? l)
                        (let ([conf (car l)])
                          (if (conf-set-member? (vector-ref states j) conf i)
                              (loop (cdr l)
                                    (append
                                     (deriv-trees conf i j enders steps names toks states nb-nts)
                                     trees))
                              (loop (cdr l) trees)))
                        trees)))
                #f)))
                      ...
                      (let* ([ender (car l)]
                        ...
                        (if ender-set
                            (let loop2 ([k (conf-set-head ender-set)]
                              ...
                              (if (>= k 0)
                                  (if (and (>= k i) (conf-set-member? (vector-ref states k) prev i))
                                      (let ([nb-prev-trees (nb-deriv-trees prev
                                                                           i
                                                                           k
                                                                           enders
                                                                           steps
                                                                           ...
                                                                           states
                                                                           nb-nts)]
                                            [nb-ender-trees (nb-deriv-trees ender
                                                                            k
                                                                            j
                                                                            enders
                                                                            steps
                  ...
                  (let loop ([l (vector-ref enders nt*)]
                    ...
                    (if (pair? l)
                        (let ([conf (car l)])
                          (if (conf-set-member? (vector-ref states j) conf i)
                              (loop (cdr l)
                                    (+ (nb-deriv-trees conf i j enders steps toks states nb-nts)
                                       nb-trees))
                              (loop (cdr l) nb-trees)))
                        nb-trees)))
                #f)))
