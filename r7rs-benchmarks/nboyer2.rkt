...
(let ()
  ...
  (define (symbol->symbol-record sym)
    (let ((x (assq sym *symbol-records-alist*)))
      (if x
          (begin
                 ; (display "SYMBOL->SYMBOL-RECORD: ")
                 ; (display (untranslate-term (cdr x)))
                 ; (newline)
                 (cdr x)
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
             ...
             (cond (temp-temp
                   ...
                   (else
                      ...
                      ; (when (list? term1)
                            ...
                            ; (display (cons term2 term1))
                            ; (newline)

                             ; (error "term was a list: " (untranslate-term term1)))
                    ...
                    ; (display term2)
                    ; (newline)
                    ; (display (untranslate-term term1))
                    ; (newline)
                    ; (display (list? (cons term2 term1)))
                    ; (newline)
  ...
  ; (add-lemma '(equal (f x) (g x)))
  ; (add-lemma '(equal (plus (plus x y) z)
  ;                  (plus x (plus y z))))

  ; (display (rewrite (translate-term '(f a))))
  ; (newline)

  ; (display (rewrite (translate-term '(plus (plus a b) c))))
  ; (newline)

  ; (display (translate-term '(plus a (plus b c))))
  ; (newline)

  ; (display (rewrite (translate-term '(loop a))))
...
; (put 'loop 'lemmas (list (list 'equal '(loop x) '(loop x))))

; (display (rewrite '(loop a)))
