...
(define (gen n)
  (let* ([n/2 (quotient n 2)]
    ...
    (define (rads-of-size n)
      (let loop1 ([ps (three-partitions (- n 1))]
                  [lst '()])
        (if (null? ps)
            lst
            (let* ([p (car ps)]
                   ...
                   [nc2 (vector-ref p 1)]
                   [nc3 (vector-ref p 2)])
              (let loop2 ([rads1 (vector-ref radicals nc1)]
                    ...
                    (let loop3 ([rads2 (if (= nc1 nc2)
                      ...
                      (if (null? rads2)
                          ...
                          (let loop4 ([rads3 (if (= nc2 nc3)
                            ...
                            (if (null? rads3)
                                lst
                                (cons (vector 'C (car rads1) (car rads2) (car rads3))
                                      (loop4 (cdr rads3) lst))))))))))))

    (define (bcp-generator j)
      (if (odd? j)
          '()
          (let loop1 ([rads1 (vector-ref radicals (quotient j 2))]
                      [lst '()])
            (if (null? rads1)
                lst
                (let loop2 ([rads2 rads1]
                            [lst (loop1 (cdr rads1) lst)])
                  (if (null? rads2)
                      lst
                      (cons (vector 'BCP (car rads1) (car rads2)) (loop2 (cdr rads2) lst))))))))

    (define (ccp-generator j)
      (let loop1 ([ps (four-partitions (- j 1))]
                  [lst '()])
        (if (null? ps)
            lst
            (let* ([p (car ps)]
                            ...
                            (if (null? rads3)
                                ...
                                (let loop5 ([rads4 (if (= nc3 nc4)
                                  ...
                                  (if (null? rads4)
                                      lst
                                      (cons
                                       (vector 'CCP (car rads1) (car rads2) (car rads3) (car rads4))
                                       (loop5 (cdr rads4) lst))))))))))))))
