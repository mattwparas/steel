...
;; TODO -> rename the variables to have an extra binding on them
;; when the get used in the let 
...
;; TODO -> add pass that rewrites special forms on the way down
;; that way an identifier that _is_ a special form will not be registered as one - just blanket
;; rewrite anything like that
(match! '(define x 10)
      ((define x 10) "matched")
      (else 'didnt-match))
