...
(define (lambda-no-args? e)
  (and (equal? (car e) 'lambda) (null? (cadr e))))
...
(define (empty-function-application? e)
  (and (equal? (length e) 1) (lambda-no-args? (car e))))
...
(define (lambda-application? e)
  (and (not (null? e)) (list? (car e)) (equal? (caar e) 'lambda)))
...
(define (constant? expr)
  (or (number? expr) (symbol? expr) (string? expr) (integer? expr) (boolean? expr) (null? expr)))

(define (constant-not-symbols? expr)
  (or (number? expr) (string? expr) (integer? expr) (boolean? expr) (null? expr)))
  ...
  (cond
    ...
    [(match-if expr)
     ...
     (if (constant? condition)
         ...
         (list 'if condition (constant-fold (caddr expr)) (constant-fold (cadddr expr))))]
...
(constant-fold '(list 1 2 3 4 (+ 1 2 3 4 (+ 4 5 (+ 7 8 9 10)))))

(constant-fold '(if #t 10 20))

(constant-fold '(if '(1 2 3 4) 20 40))

(constant-fold '(if 25 10 20))

(constant-fold '(if #t
                    (lambda (x) 10)
                    25))

(constant-fold '(lambda (x) (* x x)))
...
(define (meval expr env)
  (cond
    ...
    [(equal? (car expr) 'lambda) (MFunction (cadr expr) (caddr expr))]
    [else ;; hopefully function application?
     (define res (map (lambda (x) (meval x env)) expr))
     (cond
       [(function? (car res)) (apply (car res) (cdr res))]
       [(MFunction? (car res))
        ;; bind the arguments to the function arguments
        (define new-env (bind-args env (MFunction-bindings (car res)) (cdr res)))
        (meval (MFunction-body (car res)) new-env)])]))
...
(define (bind-args env list-of-symbols args)
  (if (or (null? list-of-symbols) (null? args))
      env
      (bind-args (hash-insert env (car list-of-symbols) (car args))
                 (cdr list-of-symbols)
                 (cdr args))))

;; bind the functions from my initial environment
;; into the scheme
(define initial-env (hash '+ + '- - '* * '/ / 'list list 'equal? equal?))
...
(define (m-atom? expr)
  (or (number? expr) (string? expr) (integer? expr) (boolean? expr) (null? expr) (symbol? expr)))
...
(define (walk expr)
  (cond
    ...
    [(empty-function-application? expr)
     (define body (transform-empty-function-application expr))
     (walk body)]
    [else (map walk expr)]))

(constant-fold (walk '(if #t
                          ((lambda () 10))
                          #f)))
...
(define (lambda-expression-application? expr)
  (if (and (>= (length expr) 2) (list? (car expr)) (equal? (car (car expr)) 'lambda))
      (car (cdr (car expr)))
      '()))
...
(define (naive-subst* expr env)
  ...
  (cond
    ...
    [(symbol? expr)
     ...
     (cond
       [(hash-contains? env expr)
        ...
        (if (constant? expr-value) expr-value expr)]
       [else expr])]
    [(lambda-expression-application? expr)
     ...
     (cons (list 'lambda (car (cdr (car expr))) (naive-subst* (caddr (car expr)) new-env))
           (cdr expr))]
    [else (map (lambda (x) (naive-subst* x env)) expr)]))

(define (naive-subst expr)
  (naive-subst* expr (hash)))

(constant-fold (naive-subst (constant-fold '((lambda (x) (* x x)) (+ 1 2 3 (+ 1 2 3))))))
...
(define (walk2 expr)
  (case expr
    ...
    [lambda-application?
     ...
     (if (constant? body)
         body
         (cons (list 'lambda (args (car expr)) body) (cdr expr)))]
    [else
     =>
     (map walk2 expr)]))
