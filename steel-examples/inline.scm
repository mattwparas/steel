...
(define (reduce op z lst)
  (cond
    [(null? lst) z] ; just pass it in as another argument
    [else
     (reduce op
...
(define (define-expr? expr)
  (if (and (list? expr) (equal? 3 (length expr)) (equal? (car expr) 'define)) (cdr expr) #f))

;; takes an expression, and if its a definition
;; puts the pair into the map
(define (insert-define-var env expr)
  (let ([p (define-expr? expr)]) (if p (hash-insert env (car p) (cadr p)) env)))

;; This gives the top level definitions
(define initial-env (reduce insert-define-var (hash) *program*))

(define (lookup sym)
  (if (hash-contains? initial-env sym) (hash-get initial-env sym) #f))
...
(define (count-argument-usages sym expr)
  (cond
    [(equal? sym expr) 1]
    [(list? expr) (apply + (map (lambda (x) (count-argument-usages sym x)) expr))]
    [else 0]))
...
(define (recursive? expr)
  ...
  (define (recursive?* expr)
    (cond
      [(list? expr) (if (equal? (car expr) ctx) #t (any (map recursive?* (cdr expr))))]
      [else #f]))
  (recursive?* expr))

(recursive? '(define loop (lambda () (loop))))
