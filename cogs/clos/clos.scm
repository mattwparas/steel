...
(struct Class-Object (name parents interfaces fields methods) #:mutable #:transparent)
(struct Interface (name methods))
(struct Class-Instance (class-object fields) #:transparent)

(define (list-subset? left right)
  (hashset-subset? (list->hashset left) (list->hashset right)))
...
(define/contract (Class name parents interfaces fields methods)
  ...
  (unless (list-subset?
           ...
           (transduce interfaces (flat-mapping Interface-methods) (into-list))
...
(define (contains-duplicates? lst)
  (not (equal? (hashset-length (apply hashset lst)) (length lst))))
...
(define (collect-fields list-of-class-objects)
  (transduce list-of-class-objects (flat-mapping Class-Object-fields) (into-list)))
...
(define/contract (get-method class-object name)
  ...
  (let ([local-method (-> class-object (Class-Object-methods) (hash-try-get name))])
...
(define/contract (%get-slot-idx class-object field-name)
  ...
  (-> class-object (Class-Object-fields) (position? field-name)))
...
(define/contract (set-slot! class-instance field-name value)
  ...
  (vector-set! (Class-Instance-fields class-instance)
...
(define/contract (%allocate-instance class-object)
  ...
  (Class-Instance class-object
                  ...
                  (apply mutable-vector (map (lambda (x) void) (Class-Object-fields class-object)))))
...
(define Object
  (Class 'Object
         ...
         (hash 'println
               (lambda (self)
                 (displayln (-> "#<"
                                (string-append (symbol->string (class-instance-name self)))
                                (string-append ">")))))))
...
(define Animal
  (Class 'Animal
         ...
         '()
         '(name color weight)
         (hash 'get-weight (lambda (self) (get-slot self 'weight)))))

;; Dog inherits from Animal, adds on the 'good-boy? field
(define Dog (Class 'Dog (list Animal) '() '(good-boy?) (hash)))
...
(define Worm
  (Class 'Worm
         ...
         (list Stinky)
         '()
         (hash 'smelly (lambda (self) "Smelly!") 'icky (lambda (self) "Icky!"))))

(define New-Worm
  (Make-Class 'Worm
              #:parents (list Dog)
              #:interfaces (list Stinky)
              #:methods (hash 'smelly (lambda (self) "Smelly!") 'icky (lambda (self) "Icky!"))))
