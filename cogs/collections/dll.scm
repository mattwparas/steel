...
(define (insert-between dlist before after data)
  ...
  (if before
      (set-dllink-next! before new-link)
      (set-dllist-head! dlist new-link))
  (if after
      (set-dllink-prev! after new-link)
      (set-dllist-tail! dlist new-link))
    new-link)
...
(define (remove-link dlist dlink)
  ...
  (let ((before (dllink-prev dlink))
        (after (dllink-next dlink)))
    (if before
        (set-dllink-next! before after)
        (set-dllist-head! dlist after))
    (if after
        (set-dllink-prev! after before)
        (set-dllist-tail! dlist before))))
 
(define (dllist-elements dlist)
  ; Returns the elements of DLIST as a list
  (define (extract-values dlink acc)
    (if dlink
        (extract-values (dllink-next dlink) (cons (dllink-content dlink) acc))
        acc))
