...
'(define hash-adjust (lambda (m key func) (define value (hash-try-get m key)) (hash-insert m key (func value))))
...
'(define query (lambda (n query-word bktree) (define root-word (BKTree-s bktree)) (define ts (BKTree-children bktree)) (define d (levenshtein root-word query-word)) (define lower ((lambda (res) (if (< res 0) (* -1 res) res)) (- d n))) (define upper (+ d n)) (define cs (filter (lambda (x) #true) (map (lambda (y) (hash-try-get ts y)) (range lower upper)))) (define ms (flatten (map (lambda (x) (query n query-word x)) cs))) (if (<= d n) (cons root-word ms) ms)))
...
'(define generate (lambda (bktree func) (define next-word (func)) (if (> 6 (string-length next-word)) (generate (insert-word bktree next-word) func) (generate bktree func))))
'(define read-to-list (lambda (lst) (define next-word (get-next-word!)) (read-to-list (cons next-word lst))))
